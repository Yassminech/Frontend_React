...
function Main() {
  ...
  const handleInputChange = (e) => {
    const updatedValues = { ...loginUser, [e.target.name]: e.target.value };
    setLoginUser(updatedValues);
  };

  const loginClick = async () => {
    try {
      const response = await axios.post("http://localhost:8000/api/auth/login", loginUser);
  ...
  useEffect(() => {
    ...
    // Check for token and isAdmin to auto-navigate if already logged in
    const token = localStorage.getItem("userData");
    const isAdmin = localStorage.getItem("isAdmin") === "true";

    if (token) {
      navigate(isAdmin ? "/home" : "/user-menu");
    }
  }, [navigate]);
          ...
          <div className="block xl:grid grid-cols-2 gap-4">
            ...
            <div className="hidden xl:flex flex-col min-h-screen">
              ...
              <div className="my-auto">
                <img alt="Illustration" className="-intro-x w-1/2 -mt-16" src={illustrationUrl} />
